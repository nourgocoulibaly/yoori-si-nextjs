...
export default function UserDashboardContent() {
  ...
  const [filteredRequests, setFilteredRequests] = useState<any[]>([]);
	...
	useEffect(() => {
		...
		const resolved = filteredRequests.filter(req => req.requestStatus === 'Résolu').length; // Correction ici
		const notResolved = filteredRequests.filter(req => req.requestStatus === 'Non résolu').length;
...
{/* </div> */}
    ...
    <CardContent className='grid gap-5'>
        {filteredRequests
            .sort((a, b) => b.createdAt?.toDate().getTime() - a.createdAt?.toDate().getTime())
            .slice(0, 5)
            .reduce((acc: { userName: string; count: number; direction?: string }[], request) => {
							const existingUser = acc.find(user => user.userName === request.userName);
							if (existingUser) {
									existingUser.count++;
							} else {
									acc.push({ userName: request.userName, count: 1, direction: request.userDirection });
							}
							return acc;
					}, [])
            .map((user, index) => (
